...
void Context::Render() {
	if (ImGui::Begin("ui window")) {
		...
		if (ImGui::CollapsingHeader("light")) {
			...
			ImGui::SliderFloat("ambient strength", &m_ambientStrength, 0.0f,
							   1.0f);
		}
	}
	ImGui::End();
	...
	auto projection = glm::perspective(
		glm::radians(45.0f), (float)m_width / (float)m_height, 0.01f, 20.0f);
	auto view =
...
bool Context::Init() {
	...
	ShaderPtr vertShader =
		Shader::CreateFromFile("./shader/lighting.vs", GL_VERTEX_SHADER);
	ShaderPtr fragShader =
		Shader::CreateFromFile("./shader/lighting.fs", GL_FRAGMENT_SHADER);
	...
	auto rot = glm::rotate(glm::mat4(1.0f), glm::radians(90.0f),
